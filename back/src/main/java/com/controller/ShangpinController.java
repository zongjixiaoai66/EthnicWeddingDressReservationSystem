    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(shangpin !=null){
            ...
            BeanUtils.copyProperties( shangpin , view );//把实体数据重构到view中
                ...
                //级联表
                SheyingshiEntity sheyingshi = sheyingshiService.selectById(shangpin.getSheyingshiId());
                if(sheyingshi != null){
    ...
    */
    ...
    public R save(@RequestBody ShangpinEntity shangpin, HttpServletRequest request){
        ...
        if(false)
            ...
            shangpin.setSheyingshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<ShangpinEntity> queryWrapper = new EntityWrapper<ShangpinEntity>()
            .eq("sheyingshi_id", shangpin.getSheyingshiId())
            .eq("shangpin_name", shangpin.getShangpinName())
            .eq("shangpin_uuid_number", shangpin.getShangpinUuidNumber())
            .eq("paishedi_types", shangpin.getPaishediTypes())
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            ShangpinEntity shangpinEntity = new ShangpinEntity();
//                            shangpinEntity.setSheyingshiId(Integer.valueOf(data.get(0)));   //摄影师 要改的
//                            shangpinEntity.setShangpinName(data.get(0));                    //作品名称 要改的
//                            shangpinEntity.setShangpinUuidNumber(data.get(0));                    //作品编号 要改的
//                            shangpinEntity.setShangpinPhoto("");//详情和图片
//                            shangpinEntity.setPaisheTime(sdf.parse(data.get(0)));          //拍摄时间 要改的
//                            shangpinEntity.setPaishediTypes(Integer.valueOf(data.get(0)));   //拍摄地 要改的
//                            shangpinEntity.setShangpinTypes(Integer.valueOf(data.get(0)));   //作品类型 要改的
