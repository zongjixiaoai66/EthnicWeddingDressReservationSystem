<template>
    <div style="backgroundImage: url('/minzuhunshayudingxitong/img/back-img-bg.jpg');background-size:cover;">
        <div class="container">
            <div class="login-form" style="backgroundColor:rgba(183, 174, 174, 0.5);borderRadius:22px">
                <h1 class="h1" style="color:#000;fontSize:28px;">民族婚纱预定系统注册</h1>
                <el-form class="rgs-form" label-width="120px">
                        ...
                        <el-form-item label="摄影师姓名" class="input" v-if="tableName=='sheyingshi'">
                            <el-input v-model="ruleForm.sheyingshiName" autocomplete="off" placeholder="摄影师姓名"  />
                        </el-form-item>
                        <el-form-item label="摄影师手机号" class="input" v-if="tableName=='sheyingshi'">
                            <el-input v-model="ruleForm.sheyingshiPhone" autocomplete="off" placeholder="摄影师手机号"  />
                        </el-form-item>
                        <el-form-item label="摄影师身份证号" class="input" v-if="tableName=='sheyingshi'">
                            <el-input v-model="ruleForm.sheyingshiIdNumber" autocomplete="off" placeholder="摄影师身份证号"  />
                        </el-form-item>
                        <el-form-item label="电子邮箱" class="input" v-if="tableName=='sheyingshi'">
                            <el-input v-model="ruleForm.sheyingshiEmail" autocomplete="off" placeholder="电子邮箱"  />
                        </el-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            login() {
                            ...
                            if((!this.ruleForm.sheyingshiName)&& 'sheyingshi'==this.tableName){
                                this.$message.error('摄影师姓名不能为空');
                                return
                            }
                             if('sheyingshi' == this.tableName && this.ruleForm.sheyingshiPhone&&(!this.$validate.isMobile(this.ruleForm.sheyingshiPhone))){
                                 this.$message.error('手机应输入手机格式');
                                 return
                             }
                            if((!this.ruleForm.sheyingshiIdNumber)&& 'sheyingshi'==this.tableName){
                                this.$message.error('摄影师身份证号不能为空');
                                return
                            }
                            if('sheyingshi' == this.tableName && this.ruleForm.sheyingshiEmail&&(!this.$validate.isEmail(this.ruleForm.sheyingshiEmail))){
                                this.$message.error("邮箱应输入邮件格式");
                                return
                            }
                this.$http({
